...
class Embedscommands(commands.Cog):
	...
	async def embedscmd(self, ctx):
		...
		emb = discord.Embed(description = 'Правила однинаковы для всех участников сервера. Соблюдайте их чтобы сделать комфортным общение всех пользователей',colour=discord.Colour.from_rgb(102, 11, 237))
	...
	async def events_subcommand(self, ctx):
		await ctx.message.delete()
		emb = discord.Embed(description='**Ивенты** - различные мини-игры на сервере.\n В них может принять участие любой желающий. За различные ивенты вы можете получить вознаграждение в виде _коинов_ разного количества.',colour=discord.Colour.from_rgb(102, 11, 237))
	...
	async def eventsnptfy_subcommand(self, ctx):
		await ctx.message.delete()
		emb = discord.Embed(description='**Для того, чтобы получать уведомления о различных ивентах, нажмите на реакцию снизу.**\n \nP.s Если вы хотите отключить уведомления, повторно нажмите на реакцию',colour=discord.Colour.from_rgb(102, 11, 237))
		await ctx.send(embed=emb)
#Add cog files
def setup(bot):
